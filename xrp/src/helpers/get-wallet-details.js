...
export default async function getWalletDetails({ client }) {
  try {
    const wallet = Wallet.fromSeed(process.env.REACT_APP_SEED); // Convert the seed to a wallet : https://xrpl.org/cryptographic-keys.html
    console.log('process.env.SEED:', process.env.REACT_APP_SEED);
    console.log('Wallet in getWalletDetails:', wallet);
    ...
    // console.log('Account:', account_data.Account);
